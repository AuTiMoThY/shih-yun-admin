...
const { data, loading, fetchData, updateStatus, deleteContact } = useAppContact();
                ...
                h(UButton, {
                    ...
                    onClick: () => {
                        ...
                        if (props.structureId) {
                            ...
                            const basePath = pathInfo.structure?.url || '/contact';
                            navigateTo(`${basePath}/edit/${row.original.id}`);
                        } else {
                            // 沒有 structureId，使用預設路徑
                            navigateTo(`/contact/edit/${row.original.id}`);
                        }
                    }
                }),
                h(UButton, {
...
const confirmDelete = async () => {
    ...
    await deleteContact(id, {
        onSuccess: () => fetchData({ structure_id: props.structureId ?? null })
    });
...
onMounted(async () => {
    await fetchData({ structure_id: props.structureId ?? null });
